                        ...
                        {data.map(pro_data => (
                            <SwiperSlide className="cursor-grab w-full">
                                <Link href={`/products/${pro_data.slug}`} className="flex justify-center rounded-lg border-4 border-gray-300 mr-5 lg:h-[430px] sm:h-[400px]  h-fit" >
                                    <div className="mx-3 mb-7  relative  w-full">
                                        <Image className="mx-auto " src={pro_data.images.featured_image} width={200} height={200} alt="product_img" />
                                        <div className="flex">
                                            <div className="my-auto">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill={reviewColor(pro_data.rating)} viewBox="0 0 24 24" stroke-width="1.5" stroke={reviewColor(pro_data.rating)} className="w-5 h-5 mr-2">
                                        ...
                                        <div className="flex flex-col justify-end mt-4">
                                            <div className="flex justify-start">
                                                {pro_data.categories.slice(0, 1).map(cat => (
                                                    <button className="lg:text-xs text-[9px] border border-gray-300 hover:bg-gray-300 hover-border-white mr-5 rounded-md px-2">{cat.name.substring(0, 15) + "..."}</button>
                                                ))}
                                            </div>
