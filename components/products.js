                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                        {data.map(pro_data => (
                            <Link href={`/products/${pro_data.slug}`} className="flex justify-center" >
                                <div class="mx-3 mb-7 border p-4 relative grid">
                                    ...
                                    <div class="flex flex-col justify-end mt-4">
                                        <div class="flex justify-start">
                                            {pro_data.categories.map(cat => (
                                                <button class="text-sm border border-gray-300 hover:bg-gray-300 hover-border-white mr-5 rounded-md px-2">{cat.name}</button>
                                            ))}
                                        </div>
                                        <div class="flex justify-between mt-4">
                                            <div class="flex">
                                                <Image class="my-auto" data-v-11f2193b="" src="https://www.lifepharmacy.com/images/express-nr.svg" width={15} height={15} />
                                                <span class="text-xs my-auto ml-3 text-[10px]">1-3 HOURS</span>
                                            </div>
                                            <button class="bg-blue-500 text-white px-4 rounded-md flex py-1">
